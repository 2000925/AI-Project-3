...
class Pieces():
    ...
    def selecting(self, p):
        ...
        if self.ar[x][y][-1] == 'k':
            res = self.Before_castling(self.ar, type)
            if res[0] == 1:
                lst.append((x,y-2))
            if res[1] == 1:
                lst.append((x,y+2))
        if self.ar[x][y][-1] == 'p':
            tmp = self.prev_move.bef_pass(self.ar, x, y, type)
            if tmp != []:
                for i in tmp:
    ...
    def Before_castling(self, ar, type):
        res = [0,0]
            ...
            for x in range(8):
                for y in range(8):
                    if ar[x][y][0] == type:
                        ...
                        if ar[x][y][-1] == 'p':
                            tmp = self.prev_move.bef_pass(ar, x, y, type)
                            if tmp != []:
                                for i in tmp:
